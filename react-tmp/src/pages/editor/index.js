            ...
            if (token.content === '__CONFLICT__') {
              return (
                <span key={tokenIndex} className={getClassNameForToken(token)}>
                  w-full{' '}
                  <span className="inline-flex bg-left-bottom bg-repeat-x bg-squiggle">flex</span>{' '}
                  items-center justify-between{' '}
                  <span className="inline-flex bg-left-bottom bg-repeat-x bg-squiggle">block</span>{' '}
                  p-6 space-x-6
                </span>
              )
            }
...
function Completion({ inView }) {
  ...
  return (
    <span className="token attr-value">
      <span className="items-baseline hidden sm:inline-flex">
      ...
      {stage === 1 && 'eal-400'}
      {(stage < 2 || stage === 6) && <span className="h-5 -mx-px border" />}
      {stage >= 2 && stage <= 5 && (
        <Fragment key={stage}>
          <span className="hidden sm:inline">{stage < 5 && ' '}</span>
          {stage < 5 && <ColorDecorator color={colors.teal[400]} />}
          {stage >= 4 && <span className="relative h-5 -mx-px border" />}
          {stage === 5 && (
            <>
              <span
          ...
          {stage === 3 && <span className="relative h-5 -mx-px border" />}
          <span
            className="inline-flex"
          ...
          {stage === 2 && <span className="relative h-5 -mx-px border" />}
          <span
            className="inline-flex"
      ...
      {typed && (
        <span className="relative z-10">
          <div className="absolute top-full left-full m-0.5 -ml-16 sm:ml-0.5 rounded-lg shadow-xl">
            <div className="relative overflow-hidden rounded-lg w-96 bg-slate-700">
              <ul className="relative py-3 leading-5 text-white">
                {completions
                  ...
                  .map((completion, i) => (
                    ...
                    >
                      <span className="flex justify-center flex-none w-5">
                        {completion[2] ? (
                          <span
...
function ColorDecorator({ color }) {
  return (
    <span
      className="relative inline-flex w-3 h-3 mr-1 rounded ring-1 ring-slate-900/30 top-px"
...
export default function EditorTools() {
  return (
    <section id="editor-tools">
      <div className="px-4 pt-4 mx-auto max-w-7xl sm:px-6 md:px-8">
        <IconContainer
          className="dark:bg-sky-500 dark:highlight-white/20"
          light={require('@/img/icons/home/editor-tools.png').default}
          dark={require('@/img/icons/home/dark/editor-tools.png').default}
        />
        <Caption className="text-sky-500">开发者工具</Caption>
        <BigText>代码提示编辑器插件.</BigText>
        <Paragraph as="div">
          <p>
            ...
            <CodeWindow className="!h-[39.0625rem]">
              <div className="flex flex-auto min-h-0">
                ...
                <div className="flex flex-col flex-auto min-w-0">
                  <CompletionDemo />
                  <div className="p-4 space-y-2 font-mono text-xs leading-6 border-t border-slate-500/30 text-slate-200">
                    <h3>Problems</h3>
                    <ul className="space-y-1 text-slate-300">
                      {problems.map((problem, i) => (
                        <li key={i} className="flex min-w-0">
                          ...
                          <p className="ml-1 truncate">{problem[0]}</p>
                          <p className="flex-none hidden sm:block text-slate-500">
                            &nbsp;{problem[1]}
                          </p>
                        </li>
                      ))}
